...
var nicImageButton = nicEditorAdvancedButton.extend({	
	addPane : function() {
		...
		var html = $.ajax({
		    ...
		    async: false
 	    	...
 	    	this.imgText = JSON.parse(html);
	    	for (this.i=0; this.i<this.imgText.item.length;this.i++) {
		    if (this.imgText.item[this.i].width == null) this.addImageH();
		    else this.addImageW();
		}
	...
	addImageH: function() {
	    this.image = new bkElement('img').setAttributes({src: this.imgText.item[this.i].url, height: this.imgText.item[this.i].height}).setStyle({margin : '4px'}).appendTo(this.pane.pane);
	    bkLib.addEvent(this.image, 'mousedown', this.submit);
	},

	addImageW: function() {
	    this.image = new bkElement('img').setAttributes({src: this.imgText.item[this.i].url, width: this.imgText.item[this.i].width}).setStyle({margin : '4px'}).appendTo(this.pane.pane);
	    bkLib.addEvent(this.image, 'mousedown', this.submit);
	},

	submit : function(e) {
		var src=e.currentTarget;
		this.pane.removePane();
